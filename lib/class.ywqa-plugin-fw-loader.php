...
if ( ! class_exists( 'YWQA_Plugin_FW_Loader' ) ) {
	...
	class YWQA_Plugin_FW_Loader {
		...
		public function register_panel() {
			...
			$admin_tabs['general'] = __( 'General', 'yith-woocommerce-questions-and-answers' );

			if ( ! defined( 'YITH_YWQA_PREMIUM' ) )  {
				$admin_tabs['premium-landing'] = __( 'Premium Version', 'yith-woocommerce-questions-and-answers' );
			}
		...
		public function action_links( $links ) {
			$links[] = '<a href="' . admin_url( "admin.php?page={$this->_panel_page}" ) . '">' . __( 'Settings', 'yith-woocommerce-questions-and-answers' ) . '</a>';


			if ( defined( 'YITH_YWQA_FREE_INIT' ) ) {
				$links[] = '<a href="' . $this->get_premium_landing_uri() . '" target="_blank">' . __( 'Premium Version', 'yith-woocommerce-questions-and-answers' ) . '</a>';
			}
		...
		public function plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data, $status ) {
			...
			if ( ( defined( 'YITH_YWQA_INIT' ) && ( YITH_YWQA_INIT == $plugin_file ) ) ||
				...
				$plugin_meta[] = '<a href="' . $this->_official_documentation . '" target="_blank">' . __( 'Plugin Documentation', 'yith-woocommerce-questions-and-answers' ) . '</a>';
			}
		...
		public function register_pointer() {
			...
			$premium_message = defined( 'YITH_YWQA_PREMIUM' )
				? ''
				: __( 'YITH WooCommerce Questions and Answers is available in an outstanding PREMIUM version with many new options, discover it now.', 'yith-woocommerce-questions-and-answers' ) .
				  ' <a href="' . $this->get_premium_landing_uri() . '">' . __( 'Premium version', 'yith-woocommerce-questions-and-answers' ) . '</a>';
			...
			$args[] = array(
				...
				'content'    => sprintf( '<h3> %s </h3> <p> %s </p>',
					__( 'YITH WooCommerce Questions and Answers', 'yith-woocommerce-questions-and-answers' ),
					__( 'In YIT Plugins tab you can find YITH WooCommerce Questions & Answers options.<br> From this menu you can access all settings of YITH plugins activated.', 'yith-woocommerce-questions-and-answers' ) . '<br>' . $premium_message
				),
